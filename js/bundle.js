...
const calc = () => {
  ...
  const activateBtns = selecor => {
    var _a;
    (_a = document.querySelector(selecor)) === null || _a === void 0 ? void 0 : _a.classList.add('calculating__choose-item_active');
  };
  const calcTotal = () => {
    let total;
...
const card = () => {
  class Card {
    ...
    render() {
      const card = this.createCardItem({
        tag: 'div',
...
const forms = (formSelector, modalTImerId) => {
  ...
  const bindPostData = form => {
    form.addEventListener('submit', e => {
      ...
      postData('http://localhost:3000/requests', obj).then(data => {
        img.remove();
        console.log(data);
      }).then(() => {
...
const modal = (modalSelector, openBtnsSelector, modalTImerId) => {
  ...
  modal.addEventListener('click', e => {
    if (e.target === modal || e.target instanceof HTMLElement && e.target.getAttribute('data-closemodal') === '') {
